...
while True:
    ...
    paragum=int(0)
    paragum_a=len(text['responses'][0]['landmarkAnnotations'])
    print ("\033[1;32;40m The top3 possible WORLD-LAND-MARK of this picture is \033[0m")
    
    while paragum < paragum_a:
        if after_call.status_code == 200:
            print(text['responses'][0]['landmarkAnnotations'][paragum]['description'])
            paragum = paragum + 1
        else:
